...
\begin{theorem}[Delta Method]
  ...
  Then,
  \[
    n^r (g(X_n) - g(c)) \longdistto \DD g(c) X.
  \]
  In particular, if \(X \sim \Normal(0, \Sigma)\), then
  \[
    n^r (g(X_n) - g(c)) \longdistto \Normal(0, \DD g(c) \Sigma \DD g(c)').
  \]
\end{theorem}
\begin{remark}
  Intuition:
