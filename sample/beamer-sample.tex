...
\begin{frame}{Theorem Environments}
  \twocolumns{0.5}{0.5}{%
    ...
    % \end{lemma}
    \begin{assumption}
      The matrix \(A\in\mathbb{R}^{n\times n}\) is symmetric and has a simple largest eigenvalue \(\lambda_1\).
    \end{assumption}
    \begin{claim}
      For any unit vector \(x\), we have \(x^\top A x \le \lambda_1\).
    \end{claim}
    \begin{proof}
    ...
    \end{proof}
      ...
      \end{remark}
      \begin{observation}
        If \(x\) is an eigenvector of \(A\), then any scalar multiple of \(x\) is also an eigenvector.
      \end{observation}
    }
  \end{frame}
